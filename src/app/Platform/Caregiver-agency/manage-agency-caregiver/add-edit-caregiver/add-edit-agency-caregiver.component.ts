...
@Component({ 
  ...
  selector: 'app-add-edit-agency-caregiver',
  templateUrl: './add-edit-agency-caregiver.component.html',
  styleUrls: ['./add-edit-agency-caregiver.component.css'],
})

export class AddEditAgencyCaregiverComponent implements OnInit {
  submitted!: boolean;
  countryList: any;
  stateList: any;
  departmentList: any;
...
this._toasterService.error(CommonErrorMessages.invalidFormat, "", {
  ...
  CreateFormGroup(): void {
    this.addEditCaregiverform = this.fb.group({
      ...
      PoliceVerification:  ["", [Validators.required,CustomValidators.noWhitespaceValidator]],
  ...
  getAgencyData(id : number):void {
    ...
    if(response.data != null) {
         this.addEditCaregiverform.controls['AgencyId'].setValue(response.data[0].AgencyId);
          }
      })
     }
  ...
  AddcaregiverAdminDetails(): any {
    ...
    if (this.addEditCaregiverAdminform.valid) {
      const model = {
        ...
        AgencyId :  this.addEditCaregiverform.controls['AgencyId'].value
      };
      this.submitted = true;
