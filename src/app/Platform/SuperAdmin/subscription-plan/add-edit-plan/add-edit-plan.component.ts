...
export class AddEditPlanComponent implements OnInit {
  ...
  CreateFormGroup(): void {
    this.planForm = this.fb.group({
      ...
      MonthlyPlan:new FormControl("", Validators.required),
      AnnualPlan:new FormControl(""),
      UserId:this.userId,
      IsMonthly:new FormControl(true),
 ...
 getUserData(id:any){
  ...
  this._managePlanService.GetSubscriptionPlanData(id).subscribe((response:any)=>{
    if(response.data !=""){
      ...
      this.planForm.controls['State'].setValue(response.data[0].State.toString());
      ...
      this.planForm.controls['IsMonthly'].setValue(response.data[0].IsMonthly);
      this.planForm.controls['IsYearly'].setValue(response.data[0].IsYearly);
      this.planForm.controls['IsActive'].setValue(response.data[0].IsActive);
     }
   })
}
...
saveSubscriptionPlan(){
  const model ={
    ...
    UserId: this.userId,
    IsMonthly: this.planForm.controls['IsMonthly'].value,
    IsYearly: this.planForm.controls['IsYearly'].value,
